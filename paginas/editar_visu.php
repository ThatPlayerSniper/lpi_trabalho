...
define('INCLUDE_CHECK', true); // Define uma constante para verificar se o ficheiro foi incluído
        ...
        <div class="card">
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && (isset($_POST['nome']) || isset($_POST['cargoMudar']))) {
                ...
                if (!empty($nome) && !empty($endereco) && !empty($user_id) && !empty($pass) && !empty($cargo)) {
                    ...
                    // First, get the current user data to compare against
                    $currentUserSql = "SELECT nome, endereco FROM utilizador WHERE id_utilizador = '$user_id'";
                    $currentUserResult = executarQuery($currentUserSql);
                    ...
                    if ($currentUserResult && $currentUserResult->num_rows > 0) {
                        ...
                        $sqlcheck = "SELECT * FROM utilizador WHERE nome = '$nome' AND id_utilizador != '$user_id'";
                        $resultadoCheck = executarQuery($sqlcheck);

                        // If username exists for another user, don't update
                        if ($resultadoCheck && $resultadoCheck->num_rows > 0) {
                            echo "<p>ERRO: O nome de utilizador já existe</p>";
                            echo "<button class='btn' onclick=\"window.location.href='editar_visu.php'\">Voltar</button>";
                            return;
                        }
